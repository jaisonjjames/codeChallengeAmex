...
const App: Application = () => {
  const [fetchResult, setFetchResult] = React.useState<Results>(defaultCache);

  React.useEffect(() => {    
      async function fetch() {
        const newFetchResult = await useCachingFetch(
  ...
  const { data: rawData, isLoading, error} = fetchResult;
